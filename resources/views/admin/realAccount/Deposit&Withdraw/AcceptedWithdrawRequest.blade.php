<x-admin-master>
    ...
    <div class="container-xxl flex-grow-1 container-p-y" style="padding-left: 85px; padding-right: 85px;">
        ...
        <h4 class="fw-bold py-3 mb-4">
            <span class="fw-light">Manage Real Accounts /</span> View Withdraw Requests
        </h4>

        <!-- Ajax Sourced Server-side -->
        <div class="card" style="padding: 10px;overflow-x: scroll;">
            <h5 class="card-header">Withdraw Requests Table</h5>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <form method="post" action="{{route('ChangeDepositStatus',$depositRequest->id)}}">
                                    @csrf
                                    @method('PUT')
                                    <input hidden type="text" name="status" value="Withdraw">

                                    <button @if($depositRequest->status =='withdrawn' ) disabled @endif class="btn btn-primary">withdrawn</button>
                                </form>
                            </td>
                            @elseif($depositRequest->status == 'Pending')
                            <td>
                            ...
                            @elseif($depositRequest->status == 'withdrawn')
                            <td><button disabled class="btn btn-primary">withdrawn</button></td>


                            @endif
                            @if($depositRequest->status == 'withdrawn')
                            <td></td>
                            @else
                            <td>
                                <form method="post" action="{{route('ChangeDepositStatus',$depositRequest->id)}}">
